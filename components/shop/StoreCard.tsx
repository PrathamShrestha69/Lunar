...
const StoreCard = ({ shoe }:{shoe:ShoeType}) => {
  if (!shoe) return null;
  ...
  return (
    <div className="flex flex-col font-[DM_Sans]">
      <Link href={`/shoe/${shoe.id}`} className="block">
        <div
          onMouseEnter={sizes}
          onMouseLeave={leaveSizes}
          className="bg-[#F5F5F5] h-[56vw] lg:h-[32vw] w-[45vw] lg:w-[26vw] items-center rounded-2xl lg:px-5 cursor-pointer"
        >
      ...
      <Link href={`/shoe/${shoe.id}`} className="text-black font-bold mt-2 block">
        <h1>{shoe.name}</h1>
        <p>€{shoe.price}</p>
      </Link>
    </div>
